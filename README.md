...
包含 NSFW 作品的分类会显示模块开关，默认关闭。开启时先显示预览警告，确认年满 18 岁后即可连续浏览并点击作品查看大图；取消或按 Escape 会保持关闭。关闭开关会立即隐藏 NSFW 作品，再次开启需重新确认。

开关状态保存在当前标签页的会话中，刷新页面后保留。NSFW 图片仅在确认开启后加载；浏览器禁用会话存储时，开关仍可使用，但刷新后恢复关闭。
